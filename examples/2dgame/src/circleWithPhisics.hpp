...
public:
  CircleWithPhysics(float radius, sf::Vector2u windowSize)
      : sf::CircleShape(radius),
        PhysicsShape(1.0f, 0.9f, 0.5f, sf::Vector2f(0, 9.81), sf::Vector2f(0, 0),
                     windowSize)
  {
  ...
  void checkBounds() override
  {
    ...
    {
      this->setPosition(this->getPosition().x < 0 ? 0 : this->windowSize.x - 2 * this->getRadius(), this->getPosition().y);
    ...
    {
      this->setPosition(this->getPosition().x, this->getPosition().y < 0 ? 0 : this->windowSize.y - 2 * this->getRadius());
